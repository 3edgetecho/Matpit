...
import '../screens/property_create/component/basic_input_field.dart';
...
class NewProjectCreateScreen extends StatefulWidget {
  const NewProjectCreateScreen({super.key, required this.id});

  final String id;

  @override
  State<NewProjectCreateScreen> createState() =>
      _NewPropertyCreateScreenState();
}

class _NewPropertyCreateScreenState extends State<NewProjectCreateScreen> {
  late AddPropertyCubit addProperty;

  _initState() {
    addProperty = context.read<AddPropertyCubit>();
    addProperty.resetData();
    if (widget.id.isNotEmpty) {
      addProperty.getEditedProject(widget.id);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: CustomAppBar(
        title: widget.id.isEmpty ? 'Add New Project' : 'Update Project',
        bgColor: whiteColor,
        showButton: true,
      ),
      body: MultiBlocListener(
        ...
        child: BlocConsumer<AddPropertyCubit, AddPropertyModel>(
          ...
          builder: (context, state) {
            ...
            if (addProperty.editedInfo2 != null) {
              return const AddPropertyForm();
            } else {
  ...
  Widget _submitButton() {
    return CreateUpdateSubmitButton(
      title: widget.id.isEmpty ? 'Submit Project' : 'Update Project',
      press: () {
        if (widget.id.isNotEmpty) {
          addProperty.updateProject(widget.id);
        } else {
          addProperty.addProject();
        }
      },
    );
  }
    ...
    return ListView(
      ...
      children: [
        ...
        // ],
         ...
         AddProjectTypeView(),
        Utils.verticalSpace(20.0),
        const BasicInputFieldProject(),
        Utils.verticalSpace(20.0),
        const AddPropertyImageView(),
        Utils.verticalSpace(20.0),
        const AddProjectVideoView(),
        Utils.verticalSpace(20.0),
        const AddProjectLocationView(),
        Utils.verticalSpace(20.0),
        // const AddProjectAminities(),
        // Utils.verticalSpace(20.0),
        // const AddPropertyNearestLocation(),
        // Utils.verticalSpace(20.0),
        const AddAdditionalProjectInformation(),
        Utils.verticalSpace(20.0),
