      ...
      return Column(
        children: [
          ...
          Container(
            ...
            child: Column(
              children: [
                const FormHeaderTitle(title: "Brochure Upload"),
...
class ProjectPDF extends StatelessWidget {
  const ProjectPDF({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<AddPropertyCubit, AddPropertyModel>(
      builder: (context, state) {
        final existImage = serviceCubit.state.project_pdf.isNotEmpty
            ? serviceCubit.state.project_pdf
            : null;

        final pickImage = state.project_pdf.isNotEmpty ? state.project_pdf : existImage;
        // print('pickImage $pickImage');
        final edit = state.addState;
        return Column(
          crossAxisAlignment: CrossAxisAlignment.center,
          children: [
            if (state.project_pdf.isEmpty && state.project_pdf.isEmpty) ...[
              GestureDetector(
                onTap: () async {
                  final image = await Utils.pickSinglePdf();
                  if (image!.isNotEmpty) {
                    serviceCubit.projectPDF(image);
                  }
                },
                child: Container(
                  height: Utils.hSize(70.0),
                  ...
                  Container(
                    ...
                    child: ClipRRect(
                      ...
                      child: Container(
                        ...
                        child:isNetworkUrl(pickImage!) ? SfPdfViewer.network(pickImage!): SfPdfViewer.file(File(pickImage!)),
                      )
                    ),
                  ),
                  Positioned(
                    ...
                    child: InkWell(
                      onTap: () async {
                        final path = await Utils.pickSinglePdf();
                        if (path != null && path.isNotEmpty) {
                          serviceCubit.projectPDF(path);
                        }
                      },
                      child: const CircleAvatar(
                        maxRadius: 16.0,
