...
class _UpdateProfileScreenState extends State<UpdateProfileScreen> {
  ...
  getExitingProfileData() {
    ...
    if(login.userInfo!.type=="user"){
      profile.nameChange(login.userInfo!.user.name??"");
      profile.emailChange(login.userInfo!.user.email??"");
      profile.phoneChange(login.userInfo!.user.phone??"");
      profile.addressChange(login.userInfo!.user.address??"");
      profile.aboutMeChange(login.userInfo!.user.userName??"");
    }else{
      ...
      profile.aboutMeChange(widget.profile.userName??"");
    }
      ...
      body: BlocListener<ProfileCubit, ProfileStateModel>(
        ...
        child: ListView(
          ...
          children: [
            ...
            login.userInfo!.type=="user"?
            ProfileImages(profilePicture: login.userInfo!.user.image??""):ProfileImages(profilePicture: widget.profile.image??""),
            spacer,
            BlocBuilder<ProfileCubit, ProfileStateModel>(
              builder: (context, state) {
                ...
                return Column(
                  ...
                  children: [
                    TextFormField(
                      initialValue: login.userInfo!.type=="user"?login.userInfo!.user.name:widget.profile.name??"",
            ...
            BlocBuilder<ProfileCubit, ProfileStateModel>(
              builder: (context, state) {
                ...
                return Column(
                  ...
                  children: [
                    TextFormField(
                      initialValue: login.userInfo!.type=="user"?login.userInfo!.user.email:widget.profile.email??"",

                      decoration: const InputDecoration(
                        hintText: 'Email',
                      ),
                      keyboardType: TextInputType.emailAddress,
                      onChanged: (String text) => profileCubit.emailChange(text),
                    ),
                    if (update is ProfileUpdateFormValidate) ...[
                      if (update.error.email.isNotEmpty)
                        ErrorText(
                          text: update.error.email.first,
                        ),
                    ]
                  ],
                );
            ...
            BlocBuilder<ProfileCubit, ProfileStateModel>(
              builder: (context, state) {
                ...
                return Column(
                  ...
                  children: [
                    TextFormField(
                      initialValue: login.userInfo!.type=="user"?login.userInfo!.user.phone:widget.profile.phone??"",
                      decoration: const InputDecoration(
                        hintText: 'Phone',
                      ),
            ...
            BlocBuilder<ProfileCubit, ProfileStateModel>(
              builder: (context, state) {
                ...
                return Column(
                  ...
                  children: [
                    TextFormField(
                      initialValue: login.userInfo!.type=="user"?login.userInfo!.user.address:widget.profile.address??"",
                      decoration: const InputDecoration(
                        hintText: 'Address',
                      ),
     ...
     Container(
        ...
        child: PrimaryButton(
            ...
            onPressed: () {
              Utils.closeKeyBoard(context);
             // profileCubit.updateAgentProfileInfo();
              profileCubit.updateUserprofileInfo();
            }),
      ):
     Container(
       margin: Utils.only(bottom: 20.0),
