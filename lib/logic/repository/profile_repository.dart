...
abstract class ProfileRepository {
  ...
  Future<Either<Failure, PropertyListResponse>> getMyProperty(
      String token);

  Future<Either<Failure, ProjectListResponse>> getMyProject(
      String token);
...
class ProfileRepositoryImp extends ProfileRepository {
  ...
  Future<Either<Failure, AgentProfileModel>> getAgentDashboardInfo(
    ...
    try {
      ...
      final data = AgentProfileModel.fromJson(result);
      return Right(data);
    } on ServerException catch (e) {
      return Left(ServerFailure(e.message, e.statusCode));
    }
  ...
  Future<Either<Failure, PropertyListResponse>> getMyProperty(
      String token) async {
    try {
      final result = await remoteDataSource.getMyProperty(token);
      final data = PropertyListResponse.fromJson(result);
      return Right(data);
    } on ServerException catch (e) {
      return Left(ServerFailure(e.message, e.statusCode));
    }
  ...
  Future<Either<Failure, ProjectListResponse>> getMyProject(
      String token) async {
    try {
      final result = await remoteDataSource.getMyProject(token);
      final data = ProjectListResponse.fromJson(result);
      print("data=>${data}");
      return Right(data);
    } on ServerException catch (e) {
      return Left(ServerFailure(e.message, e.statusCode));
