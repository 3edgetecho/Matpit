...
import '../../../data/model/product/Amenities_model.dart';
...
class CreateInfoCubit extends Cubit<CreateInfoState> {
  ...
  List<Categories>? createPropertyInfo;
  ...
  Future<void> contactUsProperty(var purpose) async {
    debugPrint('called-create-info');
    if (_loginBloc.userInfo != null &&
        _loginBloc.userInfo!.accessToken.isNotEmpty) {
      emit(CreateInfoLoading());

      final result = await _repository.contactUsProperty(
          _loginBloc.userInfo!.accessToken, purpose);
      result.fold((failuer) {
        emit(CreateInfoError(
            error: failuer.message, statusCode: failuer.statusCode));
  ...
  // Future<void> getAmenitiesInfo() async {
  //   debugPrint('called-create-info');
  //   if (_loginBloc.userInfo != null &&
  //       _loginBloc.userInfo!.accessToken.isNotEmpty) {
  //     emit(CreateInfoLoading());
  //
  //     final result = await _repository.getAmenitiesInfo(
  //         _loginBloc.userInfo!.accessToken, purpose);
  //     result.fold((failuer) {
  //       emit(CreateInfoError(
  //           error: failuer.message, statusCode: failuer.statusCode));
