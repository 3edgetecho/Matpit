...
import '../../../data/model/product/property_list_response.dart';
...
class ProfileCubit extends Cubit<ProfileStateModel> {
  ...
  Future<void> getMyProperty() async {
    if (_loginBloc.userInfo != null) {
      emit(state.copyWith(profileState: ProfileLoading()));
      final result = await _profileRepository
          .getMyProperty(_loginBloc.userInfo!.accessToken);
      result.fold(
          (failure) => emit(state.copyWith(
              profileState: ProfileError(failure.message, failure.statusCode))),
          (success) {
        propertyListResponse = success;
        emit(state.copyWith(profileState: PropertyLoaded(success)));
      });
    }
  }
  ...
  Future<void> getMyProject() async {
    if (_loginBloc.userInfo != null) {
      emit(state.copyWith(profileState: ProfileLoading()));
      final result = await _profileRepository
          .getMyProject(_loginBloc.userInfo!.accessToken);
      result.fold(
          (failure) => emit(state.copyWith(
              profileState: ProfileError(failure.message, failure.statusCode))),
          (success) {
        projectListResponse = success;
        emit(state.copyWith(profileState: ProjectLoaded(success)));
      });
    }
  }
  ...
  //   print("Sending Profile Update Request With State:\n$state");
  //   print("==================================================================");
  //
  //   final result = await _profileRepository.updateAgentProfileInfo(
  ...
  //
  //       // 🔥 Correct 422 (Validation Error)
  //       if (failure.statusCode == 404) {
  //         print("422 Validation Error: ${failure.message}");
  //
  //         emit(state.copyWith(
  //           profileState: ProfileUpdateError("All felid required", 422),
  //         ));
  //         return;
  //       }
  //
  ...
  //       emit(state.copyWith(
  //         profileState: ProfileUpdateError(failure.message, failure.statusCode),
  //       ));
  //     },
  //         (success) {
  ...
  Future<void> getUserProfile() async {
    debugPrint('called-getUserProfile');
    if (_loginBloc.userInfo != null ) {
      emit(state.copyWith(profileState: ProfileLoading()));
      final result = await _profileRepository
          .getUserProfile(_loginBloc.userInfo!.accessToken);
      result.fold((failure) {
        final errors = ProfileError(failure.message, failure.statusCode);
        emit(state.copyWith(profileState: errors));
      }, (success) {
