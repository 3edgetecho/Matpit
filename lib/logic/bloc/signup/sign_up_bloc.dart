...
class SignUpBloc extends Bloc<SignUpEvent, SignUpModelState> {
  ...
  SignUpBloc(this.repository) : super(const SignUpModelState()) {
    ...
    on<SignUpEventType>((event, emit) {
      emit(state.copyWith(
          type: event.type, state: const SignUpStateInitial()));
    });

    on<SignUpEventUserName>((event, emit) {
      emit(state.copyWith(
          username: event.username, state: const SignUpStateInitial()));
    });

    on<SignUpEventState>((event, emit) {
      emit(state.copyWith(
          stateName: event.state, state: const SignUpStateInitial()));
    });

    on<SignUpEventPhone>((event, emit) {
      emit(state.copyWith(
          phone: event.phone, state: const SignUpStateInitial()));
    });on<SignUpEventCity>((event, emit) {
      emit(state.copyWith(
          city: event.type, state: const SignUpStateInitial()));
    });
  ...
  void _submitForm(
      SignUpEventSubmit event, Emitter<SignUpModelState> emit)
  async {
