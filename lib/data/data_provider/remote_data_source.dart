...
abstract class RemoteDataSource {
  ...
  Future getHomeData();
  Future getEnquiryData();
  Future getRecommendProperty();
  Future getProjectPagination(int page);
  Future getFiveProjectWithPagination(int page);
  Future getLocationPagination(int page);
  Future getFiveLocationPagination(int page);
  Future getFeaturePagination(int page);
  Future getFiveFeaturePagination(int page);
  ...
  Future<String> createProjectRequest(AddPropertyModel data, String token);
  Future<String> contactUsProperty(String token, var purpose);

  Future<String> updatePropertyRequest(
      String id, AddPropertyModel data, String token);

  Future<String> updateProjectRequest(
      String id, AddPropertyModel data, String token);
  ...
  Future getProjectEditInfo(String id, String token);
  ...
  Future getSingleProjectDetails(String slug);

  Future getAgentDashboardInfo(String token);
  Future getMyProperty(String token);
  Future getMyProject(String token);
...
class RemoteDataSourceImp extends RemoteDataSource {
  ...
  Future getHomeData() async {
    ...
    String? id = await getIdBasedOnRole();
    final uri = Uri.parse(RemoteUrls.homeUrl+id!+(sp.getString("userId")??""));
    print(uri);
  ...
  Future<String?> getIdBasedOnRole() async {
    final prefs = await SharedPreferences.getInstance();

    // 'role' key check karo, maan lo 'vendor' ya 'user' store hota hai
    final role = prefs.getString('type');
  ...
  Future getRecommendProperty() async {
    ...
    final uri = Uri.parse(RemoteUrls.recommendProperty+id!+(sp.getString("userId")??""));
  ...
  @override
  Future getProjectPagination(int page) async {
    final headers = {'Accept': 'application/json'};

    SharedPreferences sp  = await SharedPreferences.getInstance();
    String? id = await getIdBasedOnRole();

    final uri = Uri.parse("${RemoteUrls.projectGetURL}?page=$page");

    print(uri);
    final clientMethod = client.get(uri, headers: headers);
    final responseJsonBody =
        await NetworkParser.callClientWithCatchException(() => clientMethod);
    return responseJsonBody["projects"]["data"];
  }

  @override
  Future getFiveProjectWithPagination(int page) async {
    final headers = {'Accept': 'application/json'};

    SharedPreferences sp  = await SharedPreferences.getInstance();
    String? id = await getIdBasedOnRole();

    final uri = Uri.parse("${RemoteUrls.projectGetURL}?pageCount=$page");
  ...
  @override
  Future getLocationPagination(int page) async {
    final headers = {'Accept': 'application/json'};

    SharedPreferences sp  = await SharedPreferences.getInstance();
    String? id = await getIdBasedOnRole();

    final uri = Uri.parse("${RemoteUrls.locationGetURL}?page=$page");

    print(uri);
    final clientMethod = client.get(uri, headers: headers);
    final responseJsonBody =
        await NetworkParser.callClientWithCatchException(() => clientMethod);
    return responseJsonBody["properties"]["data"];
  }

  @override
  Future getFiveLocationPagination(int page) async {
    final headers = {'Accept': 'application/json'};

    SharedPreferences sp  = await SharedPreferences.getInstance();
    String? id = await getIdBasedOnRole();

    final uri = Uri.parse("${RemoteUrls.locationGetURL}?pageCount=$page");
  ...
  @override
  Future getFiveFeaturePagination(int page) async {
    final headers = {'Accept': 'application/json'};

    SharedPreferences sp  = await SharedPreferences.getInstance();
    String? id = await getIdBasedOnRole();

    final uri = Uri.parse("${RemoteUrls.featureGetURL}?pageCount=$page");

    print(uri);
    final clientMethod = client.get(uri, headers: headers);
    final responseJsonBody =
        await NetworkParser.callClientWithCatchException(() => clientMethod);
    return responseJsonBody["featured_properties"]["data"];
  }

  @override
  Future getFeaturePagination(int page) async {
    final headers = {'Accept': 'application/json'};

    SharedPreferences sp  = await SharedPreferences.getInstance();
    String? id = await getIdBasedOnRole();

    final uri = Uri.parse("${RemoteUrls.featureGetURL}?page=$page");
  ...
  Future getHomeSlider() async {
    ...
    final uri = Uri.parse(RemoteUrls.bannerURl);
  ...
  Future<void> getAgentDashboardInfo(String token) async {
    SharedPreferences sp  = await SharedPreferences.getInstance();
    //final uri = Uri.parse(RemoteUrls.getWishListProperties(sp.getString("userId")??"","${sp.getString("type")??""}"));

    final uri = Uri.parse(RemoteUrls.getAgentDashboardInfo(sp.getString("userId")??""));

    print(uri);
    final clientMethod = client.get(uri, headers: headers);
    final responseJsonBody =
        await NetworkParser.callClientWithCatchException(() => clientMethod);
    return responseJsonBody["data"]["vendor"];
  }
  ...
  Future<void> getEnquiryData() async {
    ...
    final uri = Uri.parse(RemoteUrls.getMyEnquiry(sp.getString("userId")??""));
  ...
  @override
  Future<void> getMyProperty(String token) async {
    SharedPreferences sp  = await SharedPreferences.getInstance();
    //final uri = Uri.parse(RemoteUrls.getWishListProperties(sp.getString("userId")??"","${sp.getString("type")??""}"));

    final uri = Uri.parse(RemoteUrls.getMyProperty(sp.getString("userId")??""));
  ...
  @override
  Future<void> getMyProject(String token) async {
    SharedPreferences sp  = await SharedPreferences.getInstance();
    //final uri = Uri.parse(RemoteUrls.getWishListProperties(sp.getString("userId")??"","${sp.getString("type")??""}"));

    final uri = Uri.parse(RemoteUrls.getMyProject(sp.getString("userId")??""));
  ...
  //  final uri = Uri.parse(RemoteUrls.getWishListProperties(sp.getString("userId")??"","${sp.getString("type")??""}"));
    ...
    final url = Uri.parse(RemoteUrls.updateAgentProfileInfo(sp.getString("userId")??""));
    final headers = {
      'Accept': 'application/json',
      'Content-Type': 'application/x-www-form-urlencoded',
      'X-Requested-With': 'XMLHttpRequest',
  ...
  //  final uri = Uri.parse(RemoteUrls.getWishListProperties(sp.getString("userId")??"","${sp.getString("type")??""}"));
    ...
    final url = Uri.parse(RemoteUrls.updateUserProfileInfo(sp.getString("userId")??""));
    final headers = {
      'Accept': 'application/json',
      'Content-Type': 'application/x-www-form-urlencoded',
      'X-Requested-With': 'XMLHttpRequest',
  ...
  @override
  Future getSingleProjectDetails(String slug) async {
    final headers = {
      'Accept': 'application/json',
      'Content-Type': 'application/json'
    };
    final uri = Uri.parse(RemoteUrls.singleProjectDetailsUrl(slug));
  ...
  Future<String> addToWishlist(String token, String id) async {
    SharedPreferences sp  = await SharedPreferences.getInstance();
   // final uri = Uri.parse(RemoteUrls.getPricePlan()+(sp.getString("userId")??""));

    final uri = Uri.parse(RemoteUrls.addToWishlist(sp.getString("userId")??"", id+"/${sp.getString("type")??""}"));
    print(uri);
  ...
  Future getAgentProfile(String token) async {
    SharedPreferences sp  = await SharedPreferences.getInstance();
    final uri = Uri.parse(RemoteUrls.getAgentProfile(sp.getString("userId")??""));
  ...
  Future getUserProfile(String token) async {
    SharedPreferences sp  = await SharedPreferences.getInstance();
    final uri = Uri.parse(RemoteUrls.getuserProfile(sp.getString("userId")??""));
  ...
  async {
    ...
    final uri = Uri.parse(RemoteUrls.vendorPropertyStoreURL);
    debugPrint('create-url $uri');
    final request = http.MultipartRequest('POST', uri);
    request.fields.addAll(data.toMap());
    request.fields.addAll({
      "vendor_id":sp.getString("userId")??"",
      "approve_status":"1"
    });
    ...
    if (data.image.isNotEmpty) {
      print('featured_image ${data.image}');
      final thumbImage =
          await http.MultipartFile.fromPath('featured_image', data.image);
    ...
    if (data.galleryImage.isNotEmpty) {
      for (var i = 0; i < data.galleryImage.length; i++) {
        print("Slider=>${data.galleryImage[i].image}");
        final file = await http.MultipartFile.fromPath(
            'files[]', data.galleryImage[i].image);
        request.files.add(file);
      }
    }
    ...
    if (data.propertyVideoDto.videoThumbnail.isNotEmpty) {
      final file = await http.MultipartFile.fromPath(
          'video_image', data.propertyVideoDto.videoThumbnail);
      request.files.add(file);
    }
    ...
    if (data.propertyPlanDto.isNotEmpty) {
      for (var i = 0; i < data.propertyPlanDto.length; i++) {
        ...
        if (data.propertyPlanDto[i].planImages.isNotEmpty) {
          final file = await http.MultipartFile.fromPath(
              'floor_planning_image', data.propertyPlanDto[i].planImages);
          request.files.add(file);
        }
      }
    }
  ...
  @override
  Future<String> createProjectRequest(
      AddPropertyModel data, String token)
  async {
    final headers = postDeleteHeader;
    SharedPreferences sp  = await SharedPreferences.getInstance();
    final uri = Uri.parse(RemoteUrls.vendorProjectStoreURL);
    debugPrint('create-url $uri');
    final request = http.MultipartRequest('POST', uri);
    request.fields.addAll(data.toMap());
    request.fields.addAll({
    ...
    if (data.project_pdf.isNotEmpty) {
      print('featured_image ${data.project_pdf}');
      final thumbImage =
          await http.MultipartFile.fromPath('project_pdf', data.project_pdf);
      request.files.add(thumbImage);
    }
    ...
    if (data.galleryImage.isNotEmpty) {
      for (var i = 0; i < data.galleryImage.length; i++) {
        print("Slider=>${data.galleryImage[i].image}");
        final file = await http.MultipartFile.fromPath(
            'gallery_images[]', data.galleryImage[i].image);
        request.files.add(file);
      }
    }
    ...
    if (data.propertyPlanDto.isNotEmpty) {
      for (var i = 0; i < data.propertyPlanDto.length; i++) {
        ...
        if (data.propertyPlanDto[i].planImages.isNotEmpty) {
          final file = await http.MultipartFile.fromPath(
              'floor_plan_images[]', data.propertyPlanDto[i].planImages);
          request.files.add(file);
        }
      }
    }
      ...
      String id, AddPropertyModel data, String token) async
  {
    final headers = postDeleteHeader;
    SharedPreferences sp  = await SharedPreferences.getInstance();
    final uri = Uri.parse(RemoteUrls.updateProjectUrl(id, token));
    ...
    if (data.propertyPlanDto.isNotEmpty) {
      for (int i = 0; i < data.propertyPlanDto.length; i++) {
        ...
        if (element.isNotEmpty && !element.contains('https://')) {
          final file =
              await http.MultipartFile.fromPath('floor_plan_images[]', element);
          request.files.add(file);
        }
        // else if (element.isNotEmpty) {
        //   //final imgPath = element.split(RemoteUrls.rootUrl).last;
  ...
  Future getPropertyEditInfo(String id, String token) async {
    final headers = {'Accept': 'application/json'};
    final uri = Uri.parse(RemoteUrls.singlePropertyDetailsUrl(id));
  ...
  @override
  Future getProjectEditInfo(String id, String token) async {
    final headers = {'Accept': 'application/json'};
    final uri = Uri.parse(RemoteUrls.singleProjectDetailsUrl(id));
  ...
  @override
  Future<String> contactUsProperty(String id, var token) async {
    final headers = {'Accept': 'application/json'};
    final uri = Uri.parse(RemoteUrls.contactUsProperty(""));

    final clientMethod = client.post(uri, headers: headers, body: token);
    final responseJsonBody =
    await NetworkParser.callClientWithCatchException(() => clientMethod);

    if (responseJsonBody != null && responseJsonBody is Map<String, dynamic>) {
      String message = responseJsonBody["message"] ?? "No message";
      ...
      Fluttertoast.showToast(
        ...
        timeInSecForIosWeb: 1, // Time to show the toast on iOS and web
